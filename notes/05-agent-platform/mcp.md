...
**范围说明**：本文基于 `noc_agent/agent/tools.py`（工具层独立）、`noc_agent/mcp/server.py`（手写最小 Server）、`examples/mcp_client_demo.py`（协议演示）与 `tests/test_mcp.py`。前置阅读：[../04-agent/agent-loop.md](../04-agent/agent-loop.md) 第 5 节。
...
对本项目的直接意义：`noc_agent/mcp/server.py` 暴露的 NOC 工具，不加一行定制代码即可被 Claude Code、Cursor 等任何 MCP 客户端发现和调用，而不只服务于我们自己的 Agent。

## 2. 与手写 tool calling 的关系：同一份契约，两种消费方式

对比 `noc_agent/agent/tools.py` 中的工具定义与两种暴露格式：

| 内容 | OpenAI tools 格式（noc_agent/agent/loop.py 用） | MCP tools/list（noc_agent/mcp/server.py 用） |
|------|--------------------------------|-----------------------------------|
...
本次配套重构：工具定义从 `noc_agent/agent/loop.py` 移入独立的 `noc_agent/agent/tools.py`——一份TOOL_REGISTRY，两个消费方（Agent 循环、MCP Server）。工具与消费方之间的这条边界，正是 MCP 标准化的对象。
...
本实现用 stdio 传输：客户端把 Server 作为**子进程**拉起，每行一个 JSON 消息，stdin 进、stdout 出。这是本地 MCP Server 的标准形态（远程场景用 Streamable HTTP，本项目未实现）。`examples/mcp_client_demo.py` 展示了完整生命周期：拉起子进程 → 握手 → 发现 → 调用 → 终止。
...
{ "mcpServers": { "telecom-noc": {
    "command": "python3",
    "args": ["-m", "noc_agent.mcp.server"], "cwd": "/path/to/telecom-noc-agent" } } }
```
...
```bash
python3 examples/mcp_client_demo.py        # 协议全流程（真实调用需 8001/8002 在运行）
...
3. 给 `noc_agent/mcp/server.py` 增加一个新工具需要改几个文件？这说明工具层独立的价值是什么？
