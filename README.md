...
- [x] **v1.2 LangGraph 对比章** — 同一 Agent 用框架重写，逐概念对照笔记（State/Node/ToolNode ↔ 手写循环）
- [x] **v1.3 工单自动生成** — 首个写操作工具（意图门/幂等保护/审计三道防线），SQLite 持久化，大屏工单面板，「监控→诊断→派单」闭环
...
当前还包括：LangGraph 对比实现（可选依赖）
当前还包括：工单生成（SQLite · 写操作安全设计）
规划中：Milvus/FAISS（知识规模化）· 真实 NOC 数据源对接 · 工单状态流转与人工审批
